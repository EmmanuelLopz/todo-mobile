...
const DeleteTaskModal: React.FC<Props> = ({ visible, taskId, onDeleted, onCancel }) => {
  ...
  const handleConfirm = async () => {
    try {
      ...
      if (status === 404) {
        ...
        setError(err?.message ?? "Cannot reach the server. Check your connection.");
      } else {
      ...
      >
        ...
        >
          ...
          <View style={{ width: "75%", gap: 5 }}>
            <Button label="Delete Task" variant="danger" onPress={handleConfirm} loading={loading} disabled={loading} />
